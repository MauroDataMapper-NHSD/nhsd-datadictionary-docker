...
This section describes deploying Mauro to a server that has already been prepared with the necessary software; if this has not been done then copy the `prepare_mauro_server.sh` script to the server, read the information at the top of that file and run it.
...
2.  Edit `update-from-github.sh` to make sure the correct branch is used:
    ...
    Make sure the script is runnable with:

    ```bash
       chmod ug+x update-from-github.sh
    ```
